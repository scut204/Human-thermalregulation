...
sdno=[1 4 5  ;
      8 5 4  ];
nv1=norm_vector(C(sdno(1,:),:));
nv2=norm_vector(C(sdno(1,:),:));

perm =  [1 4 5 2 3 6 ;
         8 5 4 7 6 3 ];
nopt=1;
[~,gp]   = gauss_tri(nopt);   % get Gauss points and weights 
...
for i=1:2
           ...
           [B, ~]      = BmatHeat3D_triprism(eta,psi,omega,C(perm(i,:),:)); % derivative of the shape functions 
           X(ind,:)   =  N*C(perm(i,:),:);          % Gauss points in physical coordinates  
