function points=x_plane_cut_test(vertex,faces,p,x)
...
if isempty(num)
    ...
    for i=1:size(faces,2)
        ...
        np=0;
        for j=1:size(faces,1)     % read the dim of faces , so lines and triangle, quadrangle are all right.
            lamda=( x-v(1,j) )/( v(1,j+1)-v(1,j) );
            if(lamda<=1 && lamda >=0)
                np=np+1;
